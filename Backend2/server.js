...
//Cria uma conexão de banco de dados empresa.db se esta criado, se não, esse código criará uma db com esse nome.
...
// Realiza uma consulta de todas as informações da tabela produto
const search = (callback)=>{
 ...
 // Prepara una consulta pare adicionar dados ao nosso bd.
const insertData = db.prepare(
    `INSERT INTO Produtos (ProductName, SupplierID, Category ID, Unit, Price)
    VALUES (?, ?, ?, ?, ?)`,
    (err)=> {
        if(err){
            console.error(err);
...
// Prepara uma consulta para excluir dados do be.
const deleteData = db.prepare(
    `DELETE FROM Produtos WHERE Product ID == ?`,
    (err)=>{
        if(err){
            console.error(err);
        }else{
